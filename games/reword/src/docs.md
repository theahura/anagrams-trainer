...
- **`components/App.vue`** -- Root component, owns all game state
  ...
  - `handleKeyInput()`, `handleSubmit()`, and `handleSkip()` all guard on `state.transitioning` and `state.currentRound >= 10` to prevent input during round transitions. `handleSubmit()` uses `getSubmitFeedbackType()` to dispatch to different error messages: `'invalid-length'` shows length constraints, `'trivial-suffix'` shows "Not a true anagram", and `'wrong'` shows generic invalid message
...
- The `state.transitioning` flag in `App.vue` prevents all user input (typing, submit, skip) during the 700ms (correct) or 2500ms (skip with possible answers) delay between rounds. All three input handlers -- `handleKeyInput`, `handleSubmit`, `handleSkip` -- return early when `state.transitioning` is true. On correct answer, `App.vue` sets `flyUp=true` instead of showing a "Correct!" message text; the fly-up animation plays during the 700ms transition, then `flyUp` is reset in `advanceRound()`
