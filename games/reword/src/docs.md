...
- The `state.transitioning` flag in `App.vue` prevents input during the 700ms (correct) or 2500ms (skip with possible answers) delay between rounds. On correct answer, `App.vue` clears the message text and sets `flyUp=true`; feedback is purely visual (fly-up animation) and auditory (correct sound effect) with no text message. The fly-up animation plays during the 700ms transition, then `flyUp` is reset in `advanceRound()`
