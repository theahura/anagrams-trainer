...
- On correct answer, `App.vue` sets `flyUp=true` which triggers a CSS fly-up animation on the input tiles (translateY -40px + fade, staggered per tile via `--tile-index`) and clears the message text. Feedback is purely visual (fly-up animation) and auditory (correct sound effect) with no text message. The `flyUp` ref is reset to false in `advanceRound()`
