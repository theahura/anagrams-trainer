...
- `@/games/reword/og-image.png` is a 1200x630 PNG share image used by the OG/Twitter meta tags. It was designed via `@/games/reword/og-card.html` (a standalone HTML mockup) and can also be regenerated programmatically via `@/scripts/generate-og-images.js`. A copy lives at `@/public/reword/og-image.png` because Vite does not process `<meta content="">` as asset references -- files in `@/public/` are copied as-is to `dist/` during build, which is how the og-image ends up at the URL path declared in the meta tags
- `@/games/reword/style.css` defines the Reword game theme, tile styles, animations (including a spinning favicon loading indicator and per-tile staggered fly-up on correct answers), virtual keyboard layout for touch devices, score screen styling including countdown timer, and a `@media (pointer: fine)` block that increases vertical margins on desktop to prevent crowding without affecting mobile layout. Input area tiles use `flex: 0 1 52px` (shrinkable, no grow) so they stay on a single line even when many tiles are present (e.g., 11 tiles for long words); `height: auto` with `aspect-ratio: 1` ensures tiles remain square as they shrink below their flex-basis. On mobile (`max-width: 420px`), the flex-basis drops to 40px. Label-style elements (`#round-indicator`, `.possible-answers`, `.section-label`) share a consistent subdued treatment: small uppercase, letter-spacing, color `#818384`. The `#message` area uses Vue's `<Transition>` component for fade-in/fade-out effects (`.message-fade-*` CSS classes). All animations and transitions respect `prefers-reduced-motion`
...
- **Data flow at build time (two alternatives, same output):**
  ...
  ```
- **Difficulty progression:** 10 rounds per game: 2x3-letter roots, 3x4-letter, 3x5-letter, 1x6-letter, 1x7+-letter root
