...
- **`physics.js`** -- Exports `createPhysicsConfig()` (derived constants) and `updatePlayer()` (the per-frame simulation). Handles horizontal acceleration/deceleration, jumping (with coyote time + input buffering), variable jump height, wall sliding, wall jumping with Celeste-style control delay, gravity, AABB collision resolution (X then Y), coin collection, goal detection, and off-bottom respawn. Wall jump control delay (`wallJumpControlDelay: 0.15s`) forces horizontal input to the away-from-wall direction for a short window after a wall jump, preventing immediate momentum cancellation. The timer is ticked each frame, cleared on grounding, and set alongside `wallJumpForceDir` when a wall jump fires
