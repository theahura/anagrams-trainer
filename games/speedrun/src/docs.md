...
- **`main.js`** -- Entry point. Creates all game objects on DOMContentLoaded, runs `requestAnimationFrame` loop, manages three-state lifecycle (READY/PLAYING/COMPLETE), builds the results overlay DOM on completion. A persistent `handleKeyDown()` listener enables R key restart (mid-run or from results) and Enter key restart (from results only). Mid-run R restart increments the attempt counter and saves stats before resetting. Keyboard restart handling is deliberately separate from the movement input system in `input.js`. Also wires up path recording (record each frame, reset on restart), ghost interpolation (reads PB path for selected category from stats), and the settings UI (gear button opens modal, radio buttons for ghost category, modal blocked during PLAYING)
