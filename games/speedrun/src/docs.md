...
- **`renderer.js`** -- Exports `createRenderer(canvas)` which returns `{ render(level, player, timer, state, stats, weekSeed, ghostPos, currentPath) }`. Draws tiles, goal with glow, coins as circles, player as rectangle, and HUD. During PLAYING, also draws a ghost player (semi-transparent blue rectangle) at the interpolated PB position if `ghostPos` is provided. During COMPLETE, draws a path line overlay from the `currentPath` array. The HUD shows: week seed in top-right (on both READY and PLAYING screens), "press any key" prompt on READY, and during PLAYING: timer, coin counts, PB any% time, and attempt count
