...
- **`renderer.js`** -- Exports `createRenderer(canvas)` which returns `{ render(level, player, timer, state, stats, weekSeed) }`. Draws tiles, goal with glow, coins as circles, player as rectangle, and HUD. The HUD shows: week seed in top-right (on both READY and PLAYING screens), "press any key" prompt on READY, and during PLAYING: timer, coin counts, PB any% time, and attempt count
