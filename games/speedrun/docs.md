...
- **Keyboard guard:** `handleKeyDown()` in `main.js` returns early when an `<input>` element is focused (for the name form) or when the leaderboard modal is visible, preventing game actions while the player is typing or browsing scores. Escape closes the leaderboard modal
