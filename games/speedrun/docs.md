...
- **Physics:** `physics.js` implements acceleration-based movement, axis-separated AABB collision resolution, gravity with variable jump height (jump-cut multiplier when not holding jump), coyote time, input buffering, wall sliding, wall jumping with a short control delay, corner vaulting, dash, and sprint. Wall jump parameters are tuned for Hollow Knight-style same-wall climbing: low horizontal push-away speed (100px/s) and a very brief control override (0.04s) so the player arcs only ~7px from the wall before regaining control to hold back and re-cling. Corner vault is a Silksong-inspired forgiveness mechanic with two triggers: (1) wall-slide vault, when the player's top half clears a corner while the bottom half is still on the wall, jumping launches them forward over the platform; (2) edge vault, when the player just landed on a platform edge (within 100ms) with their hitbox overhanging the air and pressing into the platform, jumping vaults forward instead of a regular jump. Dash (tap E) is a pure cardinal-direction displacement: it saves the player's current velocity, overrides it to 500px/s in the input direction (left, right, or down), zeroes the orthogonal axis (vy=0 for horizontal dashes, vx=0 for down dash), freezes gravity for the 0.15s duration, then restores the saved pre-dash velocity when the timer expires. Horizontal input takes priority over down when both are held; if no directional input is held, dash defaults to horizontal based on current vx sign. There is no up dash. Has a 0.5s cooldown. Sprint (hold E) raises the horizontal speed cap from 250 to 375px/s while held
- **Stats persistence:** `stats.js` stores per-day personal bests in localStorage under `speedrun-stats-{daySeed}` (e.g. `speedrun-stats-2026-04-08`), tracking attempts and best times for four categories (any%, 100% red, 100% blue, 100%). Stats also store `bestPaths` -- per-category path data (arrays of `[x, y, t]` tuples) that correspond to the PB run for each category. Old stats without `bestPaths` are handled gracefully via fallback in `updatePersonalBest`
...
- **Settings:** `settings.js` persists user preferences in localStorage under `speedrun-settings`. Currently stores the ghost category preference (off/anyPercent/hundredRed/hundredBlue/hundredPercent, defaulting to anyPercent). A gear button in the HTML opens a settings modal with radio buttons. The settings modal is blocked from opening during PLAYING state, and all keydown events are suppressed while the modal is open
- **How to Play modal:** `howToPlay.js` provides a `?` button (always accessible, not gated to READY state) that opens a modal showing control instructions organized into basic controls (movement, jump, dash, sprint, restart) and advanced mechanics (wall slide/jump, corner vault). Uses localStorage key `speedrun-seen-how-to-play` for first-visit auto-open -- same pattern as Reword's `reword-seen-how-to-play`. The `setupHowToPlayModal()` function returns `{ isOpen, close }` so `main.js` can guard keyboard input and close via Escape. Follows the same `.hidden` class toggle and backdrop-click-to-close pattern as settings and leaderboard modals
- **Global leaderboard:** Firebase Firestore stores scores in `leaderboards/{daySeed}/scores/{auto-id}` with fields `name`, `time`, `category`, `weekSeed`, `submittedAt`. Submission is manual -- the player clicks "Submit to Leaderboard" on the results screen, enters a name (validated by `@/games/speedrun/src/nameFilter.js`), and each completed category is submitted as a separate Firestore document. The leaderboard modal (accessible from a persistent button in the HTML) shows the top 50 scores per category with tabbed navigation (Any%, 100% Red, 100% Blue, 100%). If the player's local PB is outside the top 50, their rank is fetched separately via a count query. The leaderboard is purely additive to the existing localStorage stats system -- it does not replace it
...
- **Keyboard guard:** `handleKeyDown()` in `main.js` returns early when an `<input>` element is focused (for the name form), when settings are open, when the how-to-play modal is open, or when the leaderboard modal is visible, preventing game actions while the player is interacting with overlays. Escape closes both the how-to-play and leaderboard modals
