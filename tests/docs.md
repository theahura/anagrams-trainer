...
- **`speedrun/physics.test.js`** -- Tests gravity, platform landing, horizontal collision, variable jump height, coyote time, input buffering, wall sliding, wall jumping, acceleration-based movement, and max fall speed clamping. Uses inline grid fixtures and a `makeLevel()` helper
- **`speedrun/level.test.js`** -- Tests grid dimensions, seed determinism, start/goal placement, coin generation, and BFS reachability (verified across multiple seeds). Also tests weekly seed consistency within a week and divergence across weeks
