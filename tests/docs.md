...
- **`components.test.js`** -- Vue component tests using `@vue/test-utils`:
  ...
  - `App` (integration): mounts the full App component with mocked fetch and localStorage. Tests timer pause/resume during HowToPlay modal, Escape key closing the modal, and timer urgency class application (uses `vi.useFakeTimers` to control time progression)
