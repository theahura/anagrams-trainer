...
- **`speedrun/leaderboard.test.js`** -- Tests leaderboard Firestore operations with mocked firebase/firestore imports: `submitScore` validation (category, time range, name), `fetchLeaderboard` query construction and result mapping, and `fetchPlayerRank` count-based ranking
