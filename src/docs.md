...
- **`game.js`** -- Puzzle selection and answer validation
  ...
  - `isValidAnswer(answer, round)` checks that the answer appears in the round's expansion lists for one of the **offered letters** AND is not a trivial extension. Only expansion keys present in `round.offeredLetters` are checked; falls back to `Object.keys(round.expansions)` if `offeredLetters` is absent (backward compatibility)
  - `getAnswersForRound(round)` returns all valid answer words for a round, filtered to only words whose expansion letter is in `round.offeredLetters`. Used by the score screen to display possible answers for skipped rounds
...
- `isValidAnswer` and `getAnswersForRound` both gate on `offeredLetters`, which is the enforcement mechanism for the spec requirement that players must use one of the offered letters. Without this, any valid expansion letter would be accepted
